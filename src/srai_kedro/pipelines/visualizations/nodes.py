...
from srai_kedro.pipelines.processing.nodes import LazyLoadedGeoDataFrames, _load_gdf
...
def plot_clustered_regions_with_roads(
    ...
    areas_lazy: LazyLoadedGeoDataFrames,
    clusters: List[int],
) -> Dict[str, Figure]:
    # areas = {k: _load_gdf(v) for k, v in areas_lazy.items()}
    area = _load_gdf(areas_lazy["Wroclaw_Poland"])  # FIXME
